...
3. OpenAI Whisper APIで文字起こし
   ↓
4. 段階的フィルタリング（機械的フィルタ + LLMフィルタ）
   ↓
5. control.pyへTCP通信でテキスト送信
   ↓
6. control.pyがLLMカットで各指標を評価
   ↓
7. 結果をCSVファイルに保存
   ↓
8. dash_server.pyがFastAPIからデータを取得
   ↓
9. ダッシュボードに表示（5秒間隔で自動更新）
```
...
- **誤作動防止**: 短すぎる音声（0.8秒未満）、空のテキスト、5文字未満のテキストを自動でスキップ
- **段階的フィルタリング**: 
  - **機械的フィルタ**: 句読点や終助詞による文の完全性チェック
  - **LLMフィルタ**: GPT-4o-miniによる発言完全性の高精度判定（不完全な発言を除外）
...
1. **mic_api.py**: 
   ...
   - 段階的フィルタリング（機械的フィルタ + LLMフィルタ）で不完全な発言を除外
...
1. **自然に話す**: マイクに向かって通常通り話してください
   - 最低0.8秒以上話す必要があります
   - 5文字未満の発言は自動的にスキップされます
   - 不完全な発言（文の途中で切れた場合など）は自動的にフィルタリングされます
...
| 変数名 | デフォルト値 | 説明 |
|--------|------------|------|
| `FILTER_MODEL` | gpt-4o-mini | LLMフィルタで使用するモデル |
| `FILTER_CONFIDENCE_THRESHOLD` | 0.6 | LLMフィルタの確信度閾値（この値以上でフィルタ判定を信頼） |
| `ENABLE_LLM_FILTER` | true | LLMフィルタの有効/無効（true/false） |
...
├── README.md               # このファイル
├── test_filtering.py       # フィルタリング機能のテスト
├── .env                    # 環境変数設定（作成が必要）
...
**エラー**: 発言したのにダッシュボードに表示されない

**解決方法**:
1. 発言が5文字以上か確認（短すぎる発言はスキップされます）
2. 文が完結しているか確認（不完全な発言はフィルタリングされます）
3. LLMフィルタを一時的に無効化する場合は、`.env`で`ENABLE_LLM_FILTER=false`に設定
4. フィルタの確信度閾値を調整する場合は、`FILTER_CONFIDENCE_THRESHOLD`の値を下げる（0.0〜1.0）
...
1. **音声入力** (mic_api.py)
   - マイク → VAD検出 → 音声バッファ → Whisper API → テキスト → 段階的フィルタリング（機械的+LLM）
...
- **LLMフィルタの無効化**: `.env`で`ENABLE_LLM_FILTER=false`に設定
- **確信度閾値の調整**: `FILTER_CONFIDENCE_THRESHOLD`の値を変更（デフォルト: 0.6）
  - 値を下げる → フィルタリングが厳しくなる（より多くの発言が除外される）
  - 値を上げる → フィルタリングが緩くなる（より多くの発言が通過する）
- **フィルタモデルの変更**: `FILTER_MODEL`で使用するモデルを変更
